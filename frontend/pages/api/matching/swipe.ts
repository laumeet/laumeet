    ...
    try {
      ...
      if (!target_user_id || !action) {
        return res.status(400).json({ 
          success: false,
          message: "target_user_id and action are required" 
        });
      }
      ...
      console.error("❌ Swipe proxy error:", err);
      
      let errorMessage = "Unable to process swipe. Please try again later.";
